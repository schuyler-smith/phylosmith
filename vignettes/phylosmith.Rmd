...
***Examples***
```{r find_unique_taxa, eval=FALSE}
find_unique_taxa(soil_column, treatment = c('Matrix', 'Treatment'), subset = 'Control')
```
```{r find_unique_taxa_run, echo=FALSE}
sapply(find_unique_taxa(soil_column, treatment = c('Matrix', 'Treatment'), subset = 'Control'), head)
```
...
merge_samples(soil_column, treatment = 'Matrix', subset = 'Manure', merge_on = 'Day')@otu_table[1:6,]
```
...
abundance_lines_ggplot(soil_column, classification = 'phylum', treatment = c('Matrix', 'Treatment'))
```
...
***Examples***
```{r prep_network_phyloseq, echo=FALSE}
network_soil_column <- taxa_filter(soil_column, treatment = c('Matrix', 'Treatment'), subset = 'Soil_Manure')
network_soil_column <- conglomerate_taxa(network_soil_column, 'phylum', taxa_are_ordered = FALSE)
```
```{r network_phyloseq}
network_phyloseq(network_soil_column, treatment = c('Matrix', 'Treatment'), subset = 'Soil_Manure', co_occurrence_table = NULL, classification = 'phylum')
```
...
***Examples***
```{r prep_nmds_phyloseq_ggplot, echo=FALSE}
nmds_soil_column <- conglomerate_taxa(soil_column, 'genus')
```
```{r nmds_phyloseq_ggplot}
nmds_phyloseq_ggplot(nmds_soil_column, c('Matrix', 'Treatment'), circle = TRUE, labels = 'Day', verbose = FALSE)
```
...
```{r prep-taxa_abundance_bars_ggplot}
ta_soil_column <- taxa_filter(soil_column, c('Matrix', 'Treatment'), frequency = .7)
```
```{r taxa_abundance_bars_ggplot}
taxa_abundance_bars_ggplot(ta_soil_column, classification = 'phylum', treatment = c('Matrix', 'Treatment'), subset = 'Control', transformation = 'mean')
```
...
***Usage***
```r
tsne_phyloseq_ggplot(phyloseq_obj, treatment, subset = NULL, merge_on = treatment)
```
***Arguments***
...
***Usage***
```r
bootstrap_rho(phyloseq_obj, treatment, subset = NULL, merge_on = treatment)
```
***Arguments***
...
### curate_cooccurrence()
<a name="curate_cooccurrence"></a>

***Usage***
```r
curate_cooccurrence(phyloseq_obj, treatment, subset = NULL, merge_on = treatment)
```
***Arguments***
