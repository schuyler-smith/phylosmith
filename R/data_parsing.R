...
conglomerate_samples <- function(phyloseq_obj, treatment, subset = NULL, 
    ...
    treatment_name <- paste(treatment, collapse = sep)
    merge_sample_levels <- as.character(unique(sort(unlist(access(phyloseq_obj,
    ...
    otu_tab <- dcast(phyloseq_table[,c('OTU','Abundance','Merged_Name'), 
        with = FALSE], Merged_Name ~ OTU, value.var = 'Abundance', 
        fun.aggregate = mean)

    sub_phy <- do.call(merge_phyloseq, 
        sapply(treatment_classes, FUN = function(group){
                group_phy <- eval(parse(text = 
                    paste0('subset_samples(taxa_filter(phyloseq_obj, 
                    treatment), ', treatment_name, ' == "', group, '")')))
    ...
    phyloseq_obj <- tryCatch({
        eval(parse(text = paste0(
            'subset_samples(phyloseq_obj, !(', treatment_name,
    ...
    phyloseq_obj <- phyloseq(
        otu_table(t(as.matrix(otu_tab[order(factor(otu_tab$Merged_Name, 
                levels = merge_sample_levels)),], 
                rownames = 'Merged_Name')), 
            taxa_are_rows = TRUE), 
        access(phyloseq_obj, 'tax_table'),
        access(phyloseq_obj, 'sam_data')[order(factor(rownames(
...
conglomerate_taxa <- function(phyloseq_obj, classification, 
    ...
    if(hierarchical){
        tax_table(phyloseq_obj) <- access(phyloseq_obj, 
            ...
            classification))]
    } else {tax_table(phyloseq_obj) <- access(phyloseq_obj, 
        'tax_table')[,classification]}

    phyloseq_table <- melt_phyloseq(phyloseq_obj)
    otus <- eval(parse(text = paste0(
    ...
    taxa <- eval(parse(text = paste0("setkey(unique(phyloseq_table[, c('",
        paste(colnames(access(phyloseq_obj, 'tax_table')), collapse = "', '"), 
        "')]), ", 
        paste(colnames(access(phyloseq_obj, 'tax_table')), collapse = ', '), 
            ")")))
...
melt_phyloseq <- function(phyloseq_obj){
    ...
    melted_phyloseq <- melt.data.table(data.table(as(access(phyloseq_obj, 
        'otu_table'), "matrix"), keep.rownames = TRUE), id.vars = 1)
    ...
    sample_data <- NULL
    if(!(is.null(access(phyloseq_obj, 'sam_data')))){
        sample_data <- data.table(data.frame(access(phyloseq_obj, 'sam_data'), 
    ...
    melted_phyloseq <- melted_phyloseq[order(melted_phyloseq$Abundance
        , decreasing = TRUE), ]
...
order_treatment <- function(phyloseq_obj, treatment, order){
    ...
    if(order[1] == 'numeric'){
        order <- as.character(sort(as.numeric(unique(as.character(
...
#'
#' Transform abundance data into relative abundance, i.e. proportional data. #' This is an alternative method of normalization and may not be appropriate #' for all datasets, particularly if your sequencing depth varies between 
#' samples.
#' @useDynLib phylosmith
...
taxa_filter <- function(phyloseq_obj, treatment = NULL, subset = NULL, frequency = 0, below = FALSE, drop_samples = FALSE){
            ...
            phyloseq_obj <- do.call(merge_phyloseq,
                apply(array(treatment_classes), 1, FUN = function(group){
                    ...
                    sub_phy <- filter_taxa(sub_phy, function(x){
                        sum(x != 0, na.rm = TRUE) <= cutoff}, TRUE)
                    return(sub_phy)}))
        } else { 
            phyloseq_obj <- do.call(merge_phyloseq,
                apply(array(treatment_classes), 1, FUN = function(group){
    ...
    if(!(is.null(subset))){
        phyloseq_obj <- prune_samples(sample_names(phyloseq_obj)[unname(apply(
            access(phyloseq_obj, 'sam_data')[,c(treatment, treatment_name)], 1,
            function(x){any(x %in% subset)}))], phyloseq_obj)
        phyloseq_obj <- filter_taxa(phyloseq_obj, function(x){
            sum(x, na.rm = TRUE) > 0}, TRUE)
    }
...
taxa_proportions <- function(phyloseq_obj, classification, treatment = NA){
    ...
    if(any(!(is.na(treatment))) & !('sample' %in% treatment)){
        ...
        class_table <- class_table[ ,c(3,4)][, lapply(.SD, sum, na.rm = TRUE), 
            by = classification]
