...
#' @param treatment Name(s) or column number(s) in the \code{\link[phyloseq:sample_data]{sample_data()}}. Function then checks if taxa seen in frequency in each treatment. If multiple \code{sample_data()} columns are given, they will be appended to the \code{sample_data()} as one column with '.' separating each.
