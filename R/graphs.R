...
network_layout_ps <-
  function (phyloseq_obj,
    ...
    if (is.null(co_occurrence_table)) {
      co_occurrence_table <-
        co_occurrence(phyloseq_obj, treatment)[rho >= 0.8 |
                                                 rho <= -0.8]
    }
    co_occurrence_table <-
      co_occurrence_table[, c('OTU_1', 'OTU_2', 'Treatment', 'rho', 'p')]
    if (!is.null(subset)) {
      co_occurrence_table <-
        co_occurrence_table[co_occurrence_table[['Treatment']] %like% subset]
    }
    colnames(co_occurrence_table)[colnames(co_occurrence_table) == 'rho'] <-
      'weight'
    co_occurrence_table$weight <- abs(co_occurrence_table$weight)
    if (!(is.null(classification))) {
      nodes <- data.table(as(access(phyloseq_obj, 'tax_table'), 'matrix'))
      nodes <-
        data.table('Node_Name' = rownames(access(phyloseq_obj, 'tax_table')), nodes)
      set(nodes, which(is.na(nodes[[classification]])), classification, 'Unclassified')
    } else {
      nodes <-
        data.table('Node_Name' = rownames(access(phyloseq_obj, 'tax_table')))
    }
    nodes <- nodes[nodes[['Node_Name']] %in%
                     c(
                       as.character(co_occurrence_table$OTU_1),
    ...
    net <-
      graph_from_data_frame(d = co_occurrence_table,
                            vertices = nodes,
                            directed = FALSE)
    net <-
      simplify(net, remove.multiple = FALSE, remove.loops = TRUE)
