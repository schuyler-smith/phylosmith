...
***Usage***
```r
common_taxa(phyloseq_obj, treatment, subset = NULL, n = 'all')
```
...
***Examples***
```{r common_taxa}
common_taxa(soil_column, treatment = c('Matrix', 'Treatment'), subset = 'Control', n = 'all')[1:35]
```
...
`treatment` | Column name as a `string` or `numeric` in the \code{\link[phyloseq:sample_data]{sample_data}}. This can be a `vector` of multiple columns and they will be combined into a new column.
...
`merge_on` | Defines which variable the data is merged according to. This needs to be a column name as a `string` or `numeric` in the \code{\link[phyloseq:sample_data]{sample_data}}. This can be a `vector` of multiple columns and they will be combined into a new column.

***Examples***
```{r samples_otu, echo=FALSE}
manure <- taxa_filter(soil_column, treatment = c('Matrix', 'Treatment'), subset = 'Manure_Control')
manure
```
```{r conglomerate_samples}
conglomerate_samples(manure, treatment = c('Matrix', 'Treatment'), subset = 'Manure', merge_on = 'Day')
```
...
`...` | any number of column names as `string`s or `numeric`s in the \code{\link[phyloseq:sample_data]{sample_data}} that are to be combined.

***Examples***
```{r merge_treatments}
merge_treatments(soil_column, 'Matrix', 2, 'Day')
```
...
-------------------- | ------------------------------------------------------------
`phyloseq_obj` | A \code{\link[phyloseq]{phyloseq-class}} object. It must contain \code{\link[phyloseq:sample_data]{sample_data()}}) with information about each sample, and it must contain \code{\link[phyloseq:tax_table]{tax_table()}}) with information about each taxa/gene.
`sort_by` | Column name as a \code{string} or \code{numeric} in the \code{\link[phyloseq:sample_data]{sample_data}}, or vector of sample names or indices in particular order.
...
-------------------- | ------------------------------------------------------------
`phyloseq_obj` | A \code{\link[phyloseq]{phyloseq-class}} object. It must contain \code{\link[phyloseq:sample_data]{sample_data()}}) with information about each sample, and it must contain \code{\link[phyloseq:tax_table]{tax_table()}}) with information about each taxa/gene.
`treatment` | Column name as a `string` or `numeric` in the \code{\link[phyloseq:sample_data]{sample_data}}.
`order` | The order of factors in \code{treatment} column as a `vector` of `string`s. If assigned "numeric" will set ascending numerical order.
...
***Examples***
```{r unique_taxa}
unique_taxa(soil_column, treatment = c('Matrix', 'Treatment'), subset = 'Control')
```
