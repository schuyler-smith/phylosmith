...
***Usage***
```r
bootstrap_rho(phyloseq_obj, treatment, replicate_samples = 'independent', permutations = 100, cores = 0)
```
...
```{r bootstrap_rho}
bootstrapped_rhos <- bootstrap_rho(soil_column, treatment = c('Matrix', 'Treatment'), 'Day', permutations = 5)
bootstrapped_rhos
```
...
## quantile_bootstrapped_rhos
<a name="quantile_bootstrapped_rhos"></a>

Calculate quantiles for the bootstrapped rho values from the Spearman-rank co-occurrence. Can be done by treatments or for the overall data. This looks at both tails of the distribution, so it will show the p/2 cutoffs on both ends.

<br>
***Usage***
```r
quantile_bootstrapped_rhos(bootstrapped_rhos, p = 0.05, by_treatment = TRUE)
```
<br>
***Arguments***
...
Call | Description
-------------------- | ------------------------------------------------------------
`bootstrapped_rhos` | A \code{data.table} output from \code{\link[=bootstrap_rho]{bootstrap_rho}}.
...
```{r quantile_bootstrapped_rhos}
quantile_bootstrapped_rhos(bootstrapped_rhos, p = 0.05)
quantile_bootstrapped_rhos(bootstrapped_rhos, p = 0.05, by_treatment = FALSE)
```
...
## histogram_bootstrapped_rhos
<a name="histogram_bootstrapped_rhos"></a>
...
***Usage***
```r
histogram_bootstrapped_rhos(bootstrapped_rhos, p = 0.05, zeros = FALSE, x_breaks = 0.25, colors = 'default')
```
<br>
***Arguments***
...
```{r histogram_bootstrapped_rhos}
histogram_bootstrapped_rhos(bootstrapped_rhos[Treatment %in% c('Soil_Manure', 'Water_Manure')], p = 0.05)
```
